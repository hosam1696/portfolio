    ...
    "bio": {
        ...
        "skills": [
            ...
            for (let [key, url] of Object.entries(this.contacts)) {
                ...
                //     inlineList.append(`<li>${getSvgIcon(key)}<span class="vertical-sep"></span><a href="mailto:${url}">${url}</a></li>`);
                // } else
                
                if (key === 'mobile' || key === 'location') {
                    continue;
                } else {
                    inlineList.append(`<li class="line"><a href="${key === 'primary_email' || key === 'secondary_email' ?'mailto:'+url : url}" target="noopener">${getSvgIcon(key)}</a></li>`);
                }
            }
            contactsSection.append( inlineList);
        }
